...
def analyze_genre_search_consistency(
    ...
    for genre, search_results_json in results:
        ...
        metrics['min_from_centroid'] = np.min(distances_from_centroid)
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig('results/consistency/genre_centroid_distance.png', bbox_inches='tight', dpi=300)
    plt.close()

    # 2) Pairwise Distance Plot
    plt.figure(figsize=(12, 6))
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig('results/consistency/genre_pairwise_distance.png', bbox_inches='tight', dpi=300)
    plt.close()
